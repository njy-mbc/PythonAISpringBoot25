...
public class RestReqController {
    @Autowired  // Webclient webclient = new WebClient();
    ...
    public String serviceRequest(MultipartFile file, String message) {
        ...
        String result = webClient.post().uri("/detect")
                // POST방식으로 요청, 엔드포인트는 /detect
                .contentType(MediaType.MULTIPART_FORM_DATA)
                .body(BodyInserters.fromMultipartData(bodyBuilder.build())) // 본문 에서 폼 데이터 요청 실행
